...
export class GamepadComponent implements OnInit, OnDestroy {
    ...
    private extractDpadData(
        ...
        if (mapping === 'standard') {
            ...
            this.upDPadButton = this.upDPadButtonRaw < 0.2 ? 0.2 : this.upDPadButtonRaw;

            this.downDPadButtonRaw = buttons[13].value;
            this.downDPadButton = this.downDPadButtonRaw < 0.2 ? 0.2 : this.downDPadButtonRaw;

            this.leftDPadButtonRaw = buttons[14].value;
            this.leftDPadButton = this.leftDPadButtonRaw < 0.2 ? 0.2 : this.leftDPadButtonRaw;

            this.rightDPadButtonRaw = buttons[15].value;
            this.rightDPadButton = this.rightDPadButtonRaw < 0.2 ? 0.2 : this.rightDPadButtonRaw;
        } else if (mapping === '') {
    ...
    private extractMainButtonsData(buttons: ReadonlyArray<GamepadButton>) {
        ...
        this.bottomButton = this.bottomButtonRaw < 0.2 ? 0.2 : this.bottomButtonRaw;
        this.rightButtonRaw = buttons[1].value;
        this.rightButton = this.rightButtonRaw < 0.2 ? 0.2 : this.rightButtonRaw;
    ...
    private extractOptionButtonData(
        ...
        this.leftOptionButton = this.leftOptionButtonRaw < 0.2 ? 0.2 : this.leftOptionButtonRaw;
        this.rightOptionButtonRaw = rightOptionButton;
        this.rightOptionButton = this.rightOptionButtonRaw < 0.2 ? 0.2 : this.rightOptionButtonRaw;
    }
    ...
    private extractTriggerData(
        ...
        this.rightTriggerOpacity =
            this.rightTriggerOpacityRaw < 0.2 ? 0.2 : this.rightTriggerOpacityRaw;
    }

    private extractedStickData(axes: ReadonlyArray<number>, mapping: GamepadMappingType) {
        let rightStickX: number;
        let rightStickY: number;

        this.leftStickPosition.x = axes[0] * 12 + 113;
        this.leftStickPosition.y = axes[1] * 12 + 160;
        this.leftStickOpacityRaw = Math.sqrt(Math.pow(axes[0], 2) + Math.pow(axes[1], 2));
        this.leftStickOpacity = this.leftStickOpacityRaw < 0.2 ? 0.2 : this.leftStickOpacityRaw;
        ...
        this.rightStickOpacityRaw = Math.sqrt(Math.pow(rightStickX, 2) + Math.pow(rightStickY, 2));
        this.rightStickOpacity = this.rightStickOpacityRaw < 0.2 ? 0.2 : this.rightStickOpacityRaw;
    }
